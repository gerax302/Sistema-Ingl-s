<?php 
	...
	if (isset($_SESSION["sesion"])){
		...
		if($row['unidad']=="1"){
			...
			//pasarla a formato dia, mes, año
			$fecha1Inicio = strftime("%d-%m-%Y", strtotime($fecha1Inicio));
			//obtener la fecha
			$fecha1Fin=$row['desarrolloTermino'];
			//pasarla a formato dia, mes, año
			$fecha1Fin = strftime("%d-%m-%Y", strtotime($fecha1Fin));
			
	
		}
		if($row['unidad']=="2"){
			$fecha2Inicio=$row['desarrolloInicio'];
			$fecha2Inicio = strftime("%d-%m-%Y", strtotime($fecha2Inicio));
			$fecha2Fin=$row['desarrolloTermino'];
			$fecha2Fin = strftime("%d-%m-%Y", strtotime($fecha2Fin));
		}
		if($row['unidad']=="3"){
			$fecha3Inicio=$row['desarrolloInicio'];
			$fecha3Inicio = strftime("%d-%m-%Y", strtotime($fecha3Inicio));
			$fecha3Fin=$row['desarrolloTermino'];
			$fecha3Fin = strftime("%d-%m-%Y", strtotime($fecha3Fin));
		}
		if($row['unidad']=="4"){
			$fecha4Inicio=$row['desarrolloInicio'];
			$fecha4Inicio = strftime("%d-%m-%Y", strtotime($fecha4Inicio));
			$fecha4Fin=$row['desarrolloTermino'];
			$fecha4Fin = strftime("%d-%m-%Y", strtotime($fecha4Fin));
		}
		if($row['unidad']=="5"){
			$fecha5Inicio=$row['desarrolloInicio'];
			$fecha5Inicio = strftime("%d/%m/%Y", strtotime($fecha5Inicio));
			$fecha5Fin=$row['desarrolloTermino'];
			$fecha5Fin = strftime("%d-%m-%Y", strtotime($fecha5Fin));
		}
		if($row['unidad']=="6"){
			$fecha6Inicio=$row['desarrolloInicio'];
			$fecha6Inicio = strftime("%d-%m-%Y", strtotime($fecha6Inicio));
			$fecha6Fin=$row['desarrolloTermino'];
			$fecha6Fin = strftime("%d-%m-%Y", strtotime($fecha6Fin));
		}
		if($row['unidad']=="7"){
			$fecha7Inicio=$row['desarrolloInicio'];
			$fecha7Inicio = strftime("%d-%m-%Y", strtotime($fecha7Inicio));
			$fecha7Fin=$row['desarrolloTermino'];
			$fecha7Fin = strftime("%d-%m-%Y", strtotime($fecha7Fin));
		}
		if($row['unidad']=="8"){
			$fecha8Inicio=$row['desarrolloInicio'];
			$fecha8Inicio = strftime("%d-%m-%Y", strtotime($fecha8Inicio));
			$fecha8Fin=$row['desarrolloTermino'];
			$fecha8Fin = strftime("%d-%m-%Y", strtotime($fecha8Fin));
			$ultimaFecha=$row['desarrolloTermino'];	
		}
		if($row['unidad']=="9"){
			$fecha9Inicio=$row['desarrolloInicio'];
			$fecha9Inicio = strftime("%d-%m-%Y", strtotime($fecha9Inicio));
			$fecha9Fin=$row['desarrolloTermino'];
			$fecha9Fin = strftime("%d-%m-%Y", strtotime($fecha9Fin));
		}
		if($row['unidad']=="10"){
			$fecha10Inicio=$row['desarrolloInicio'];
			$fecha10Inicio = strftime("%d-%m-%Y", strtotime($fecha10Inicio));
			$fecha10Fin=$row['desarrolloTermino'];
			$fecha10Fin = strftime("%d-%m-%Y", strtotime($fecha10Fin));
		}
		...
		if($ultimaFecha!="" &&interval_date($ultimaFecha, $fecha)<0){
		     
		echo "<script type='text/javascript'>  
				confirm('La fecha limite para primeras oportunidades expiro. Favor de ingresar calificaciones de segundo oportunidad.');
				window.location='Alta_Calificaciones2.php?idgr=$agarraId&grupillos=$grupillos';
				</script>";
...
<body>
<section>
  <h2> <div align="center">Ingresar Calificaciones de Primera Oportunidad</div></h2>
 <form method="post" name="form" action="Logica_Alta_Calificaciones1.php?tipoconsulta=alta">
    <p>&nbsp;</p>
...
<input type="text" name="nivgr" id="nivgr" value="" hidden>
	...
	$agarraId=$_GET['idgr'];?>
     ...
     &grupo=<?php echo $agarraId;?>&nivel=<?php echo $nivelado;?>&grupillos=<?php echo $grupillos;?>&tipoconsulta=alta1">
    <p>&nbsp;</p>
    ...
    <td align="center" onClick="aplazaTiempo()">Unidad 1&nbsp;
    <span class="letrasfechas"><?php echo $fecha1Inicio."<->".$fecha1Fin ?></span></td>  
    <td align="center">Unidad 2&nbsp;
    <span class="letrasfechas"><?php echo $fecha2Inicio."<->".$fecha2Fin ?></span></td>
    <td align="center">Unidad 3&nbsp;
    <span class="letrasfechas"><?php echo $fecha3Inicio."<->".$fecha3Fin ?></span></td> 
    <td align="center">Unidad 4&nbsp;
    <span class="letrasfechas"><?php echo $fecha4Inicio."<->".$fecha4Fin ?></span></td>
    <td align="center">Unidad 5&nbsp;
    <span class="letrasfechas"><?php echo $fecha5Inicio."<->".$fecha5Fin ?></span></td>  
    <td align="center" on>Unidad 6&nbsp;
    <span class="letrasfechas"><?php echo $fecha6Inicio."<->".$fecha6Fin ?></span></td>  
    <td align="center">Unidad 7&nbsp;
    <span class="letrasfechas"><?php echo $fecha7Inicio."<->".$fecha7Fin ?></span></td>
    <td align="center">Unidad 8&nbsp;
    <span class="letrasfechas"><?php echo $fecha8Inicio."<->".$fecha8Fin ?></span></td> 
    <td align="center">Unidad 9&nbsp;
    <span class="letrasfechas"><?php echo $fecha9Inicio."<->".$fecha9Fin ?></span></td>
    <td align="center">Unidad 10&nbsp;
    <span class="letrasfechas"><?php echo $fecha10Inicio."<->".$fecha10Fin ?></span></td>        
     </tr>
